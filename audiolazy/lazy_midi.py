...
def str2octave(note_string):
  ...
  accident2delta = {"b": -1, "#": 1, "x": 2}
  accidents = list(it.takewhile(lambda el: el in accident2delta, data[1:]))
  octave_delta = int(data[len(accidents) + 1:]) - 1
  #print octave_delta
  return octave_delta # Octave

@elementwise("note_string", 0)
def str2octave1(note_string):
  """
  Given a note string name (e.g. "Bb4"), returns its MIDI pitch number.
  """
  if note_string == "?":
...
def str2freq(note_string):
  ...
  """
  return midi2freq(str2midi1(note_string))
...
def midi2str1(midi_number):
  ...
  names = ["C", "C", "D", "D", "E", "F", "F", "G", "G", "A", "A", "B"]
  accidents = ["", "#", "", "#", "", "", "#", "", "#", "", "#", ""]
  names = names[rnote] + octave + accidents[rnote]
  if abs(error) < 1e-4:
    return names
  else:
    err_sig = "+" if error > 0 else "-"
